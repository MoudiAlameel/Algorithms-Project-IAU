...
# ===========================================================
# 1. Load CSV (UCI student-mat.csv with ; separator)
# ===========================================================

def load_csv_colab_semicolon():
    print(" Please upload student-mat.csv ...")
    ...
    with open(filename, 'r') as file:
        reader = csv.reader(file, delimiter=';')  # <-- IMPORTANT
...
def encode_dataset(data, header):
    ...
    # تأكدنا الآن أن "G3" موجودة في الهيدر
    if "G3" not in header:
    ...
    for row in data:
        ...
        for i, value in enumerate(row):
            # نتخطى G3 لأنها الهدف (target)
            if i == g3_index:
                continue

            # نحاول نخلي القيمة رقمية
            try:
                new_row.append(float(value))
            except:
                # لو قيم نصية (categorical) نحولها إلى أرقام
                new_row.append(hash(value) % 500)

        # نحدد هدف Pass/Fail بناءً على G3
...
def std(col):
    ...
    return math.sqrt(variance) if variance > 0 else 1e-6  # نتجنب صفر
...
def train_naive_bayes(training_data):
    ...
    for c, rows in separated.items():
        summaries = []
        cols = list(zip(*rows))
        # كل الأعمدة ما عدا آخر واحد (اللابيل)
        for col in cols[:-1]:
...
raw_data, header = load_csv_colab_semicolon()
